...
export const InputRoute: Component = () => {
  ...
  const [globalValue, setGlobalValue] = createSignal('World');
  ...
  return (
    <Container tag="article">
      ...
      <Container variant="contained" color="surface">
        ...
        <Divider color="gradient" />
        <For each={variants}>
          {(variant) => <Input label="Username" class="m-1" icon="mdi:account" variant={variant} />}
        </For>
      </Container>
      <Container>
        ...
        <Input
          ...
          icon="mdi:hand-wave"
          value={globalValue()}
          onChange={(value) => setGlobalValue(value)}
        />
        <p>Saying hello to: {globalValue()}</p>
      </Container>
    </Container>
  );
};
